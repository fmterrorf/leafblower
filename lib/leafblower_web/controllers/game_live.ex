defmodule LeafblowerWeb.GameLive do
    ...
    ~H"""
      ...
      <%= if !@is_leader? do%>
        <%= if @has_answered? do %>
          <b>You picked</b>
          <div class="card-container">
            <%= render_cards(player_answer_cards_to_info(@round_player_answers[@player_id]), "light") %>
          </div>
  ...
  defp render_round_ended(
    ...
    <div>
      ...
      <%= if @has_answers? do %>
        ...
        <ul class="card-container">
          <%= for player <- Enum.map(@active_players, fn id -> @player_info[id] end),
                  cards = @round_player_answers[player.id] |> IO.inspect(label: "WHT") do %>
                <%= render_cards(player_answer_cards_to_info(cards), "light", [
                  id: player.id,
                  class: "multi-card-wrapper pointer",
                  phx_click: "pick_winner",
                  phx_value_id: player.id]) %>
          <% end %>
        </ul>
        <% else %>
      ...
      <ul class="card-container">
          <%= for player <- Enum.map(@active_players, fn id -> @player_info[id] end),
                  cards = player_answer_cards_to_info(@round_player_answers[player.id]) do %>
  ...
  defp render_winner(winner_cards, winner_player, player_score, player_info, current_user_id, is_leader?) do
    assigns = %{
      is_leader?: is_leader?,
      winner_player: winner_player,
      player_score: player_score,
      player_info: player_info,
      winner_cards: winner_cards |> IO.inspect(label: "winner")
    }

    ~H"""
    <p>And the winner for this round is <b> <%= @winner_player.name %> </b> 🎉🎉🎉 </p>
    <div class="card-container">
      <%= render_cards(player_answer_cards_to_info(@winner_cards), "light") %>
    </div>
    <%= if @is_leader? do%>
      <button phx-click="start_round" >Start Next Round</button>
    <% end %>
  ...
  defp player_answer_cards_to_info(cards) do
